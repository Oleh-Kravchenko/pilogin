...
static int open_file(MainContext* main_context, const char* filename) {
...
#if 1
	main_context->pict.screen = SDL_SetVideoMode(video_codec_context->width, video_codec_context->height, 32, SDL_SWSURFACE | SDL_FULLSCREEN);
	...
	main_context->pict.bmp = SDL_CreateYUVOverlay(main_context->pict.screen->w, main_context->pict.screen->h,
...
static void schedule_refresh(MainContext* main_context, int delay) {
	...
	timer = SDL_AddTimer(delay, sdl_refresh_timer, main_context);
}
