...
#include <string.h>
#include <SDL/SDL.h>
#include "SDL_terminal_private.h"
...
SDL_CreateTerminal (void)
{
    ...
    terminal->event_filter = SDL_GetEventFilter();
    SDL_SetEventFilter (SDL_TerminalEventFilter);
    return terminal;
}
...
int SDL_TerminalEventFilter (const SDL_Event *event)
{
    ...
    if (SDL_ActiveTerminal) {
        ...
        if ((SDL_ActiveTerminal->event_filter != NULL) && (!status))
            return (SDL_ActiveTerminal->event_filter)(event);
    }
    return status;
}
 ...
 */
int
SDL_TerminalProcessEvent (SDL_Terminal *terminal, const SDL_Event *event)
{
 ...
 */
int
SDL_TerminalEvent (SDL_Terminal *terminal, const SDL_Event *event)
{
    if (!terminal->visible)
        return 1;
    ...
    case SDL_KEYDOWN:
        if (!terminal->active)
            ...
            if (terminal->line_pos < strlen(terminal->line)) {
                SDL_TerminalEraseCursor (terminal);
                int dx = +1;
                if ((terminal->line_pos < strlen(terminal->line)) && terminal->line[terminal->line_pos] == '\t')
                    dx = terminal->tabsize;
                terminal->cpos.x += dx;
            ...
            if (event->key.keysym.sym == SDLK_a) {
                ...
                /* go to end of line */
                SDL_TerminalEraseCursor (terminal);
                while (terminal->line_pos < strlen(terminal->line)) {
                    int i, dx = 1;
                    if (terminal->line[terminal->line_pos] == '\t')
                        dx = terminal->tabsize;
                    for (i=0; i<dx; i++) {
...
SDL_TerminalBlit (SDL_Terminal *terminal)
{
    ...
    if (!(screen->flags & SDL_OPENGL)) {
        ...
        return SDL_BlitSurface (terminal->surface, &src, screen, &dst);
    }
    return 0;
}
...
SDL_TerminalClear (SDL_Terminal *terminal)
{
    ...
    /* Outer border */
    color = SDL_MapRGBA (fmt, terminal->br_color.r,terminal->br_color.g, terminal->br_color.b, terminal->br_color.unused);
    ...
    /* Inner background */
    color = SDL_MapRGBA (fmt, terminal->color.r, terminal->color.g, terminal->color.b, terminal->color.unused);
...
SDL_TerminalSetBorderColor (SDL_Terminal *terminal, int red, int green, int blue, int alpha)
{
    ...
    terminal->br_color.unused = alpha;
    return 0;
}
...
SDL_TerminalReset (SDL_Terminal *terminal)
{
    ...
    SDL_TerminalSetForeground (terminal,
                               ...
                               terminal->default_fg_color.unused);
...
{   
    ...
    terminal->default_fg_color.unused = alpha;
    return 0;
}
...
{   
    ...
    terminal->default_bg_color.unused = alpha;
    return 0;
}
...
{   
    ...
    terminal->fg_color.unused = alpha;
    return 0;
}
...
{   
    ...
    terminal->bg_color.unused = alpha;
    return 0;
}
    ...
    if (text_surf->format->BytesPerPixel == 4) {
        ...
        for (i=0; i<terminal->glyph_size.w; i++) {
            for (j=0; j<terminal->glyph_size.h; j++) {
                Uint8 *s = (Uint8 *) (terminal->surface->pixels + (x+i)*4 + (y+j)*terminal->surface->pitch);
                Uint8 *t = (Uint8 *) (text_surf->pixels + i*4 + j*text_surf->pitch);
                if (*(t+3) > *(s+3))
                    *(s+3) = *(t+3);
            }
        }
